...
class MultiLogicAdapter(LogicAdapter):
    ...
    def process(self, statement):
        ...
        result = None
        max_confidence = -1
        #print("mutil输入",statement)
        for adapter in self.get_adapters():
           # print("muadapter",adapter)
            if adapter.can_process(statement):
               # print("muTrue",)
                output = adapter.process(statement)
               # print("muoutput",output)
                results.append((output.confidence, output, ))
               # print("muresults",results)
                self.logger.info(
                ...
                if output.confidence > max_confidence:
                    #print("muoutput.confidence",output.confidence)
                    result = output
        ...
        if len(results) >= 3:
           ...
           # print("mustatements",statements)
            count = Counter(statements)
           # print("mucount",count)
            most_common = count.most_common()
           # print("mumost_common",most_common)
            if most_common[0][1] > 1:
                result = most_common[0][0]
                max_confidence = self.get_greatest_confidence(result, results)
        #print("mumax_confidence",max_confidence)
