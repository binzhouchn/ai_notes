...
class ChatBot(object):
    ...
    def sn_get_response(self, input_item, source_type=None):
        """
        Return the bot's response based on the input.

        :param input_item: An input value.
        :param conversation_id: The id of a conversation. 暂时没用
        ...
        # 走自己定义的logic，robotbestmatch
        question = eval(self.input.process_input_statement(input_item).text)['qq']

        # # Preprocess the input statement 预处理模块暂时用不到
        # for preprocessor in self.preprocessors:
        #     input_statement = preprocessor(self, input_statement)

        bot_answer = self.sn_generate_response(question, source_type)
        ...
        return self.output.process_response(bot_answer, source_type)

    def sn_generate_response(self, input_statement, source_type=None):
        response = sn_robotbest_match(input_statement, source_type)
        return response

    # 这个是chatterbot自己内置的get_response
